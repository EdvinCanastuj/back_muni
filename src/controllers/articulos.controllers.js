...
const getArticulos2 = async (req, res) => {
  try {
    const connection = await getConnection();
...
const getArticuloU = async (req, res) => {
  try {
    ...
    const connection = await getConnection();
    const result = await connection.query("SELECT a.id_articulo, a.codigo, a.cantidad, a.nombre_articulo, a.valor_unitario, a.valor_total, a.valor_baja, a.observaciones, u.nombre, u.apellido, c.nombre_cargo, d.nombre_dependencia FROM articulos a INNER JOIN usuario u ON a.id_usuario = u.id_usuario INNER JOIN cargo c ON u.id_cargo = c.id_cargo INNER JOIN dependencia d ON c.id_dependencia = d.id_dependencia WHERE a.id_usuario =?;", [id_usuario]);
    res.json(result);
  } catch (error) {
    res.status(500).send(error.message);
  }
...
const getArticuloQRVer = async (req, res) => {
  try {
    const connection = await getConnection();
    const result = await connection.query("SELECT a.id_articulo, u.nombre, a.nombre_articulo, a.codigo, a.no_serie FROM articulos a INNER JOIN usuario u ON a.id_usuario = u.id_usuario;");
    res.json(result);
  } catch (error) {
    res.status(500).send(error.message);
  }
...
const getArticuloQR = async (req, res) => {
  try {
    const { id_usuario } = req.params;
    const connection = await getConnection();
    const result = await connection.query("SELECT qr FROM articulos WHERE id_articulo = ?;", [id_usuario]);
    res.json(result);
  } catch (error) {
    res.status(500).send(error.message);
  }
...
const updateArticulo = async (req, res) => {
  try {
    ...
    const valor_total = valor_unitario * cantidad;
    const articulo = {
      id_usuario,
      codigo,
      nombre_articulo,
